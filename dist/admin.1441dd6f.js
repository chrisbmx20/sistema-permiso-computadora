...
function addTask(tsktitle, tskPriority) {
    ...
    let editBtn = btnContainer.firstChild;
    let deleteBtn = btnContainer.childNodes[1];
    if (tskPriority === "low") priority.classList.add("bg-success");
    else if (tskPriority === "medium") priority.classList.add("bg-warning");
    else priority.classList.add("bg-danger");
...
function checkDots(dot) {
    if (dot.classList.contains("bg-success")) return "low";
    else if (dot.classList.contains("bg-warning")) return "medium";
    else return "high";
}
...
//# sourceMappingURL=admin.1441dd6f.js.map
